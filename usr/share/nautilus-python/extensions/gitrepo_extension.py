# intentional-log: extension load and launch failures are only observable via Nautilus logs.
...
class GitRepoExtension(GObject.GObject, Nautilus.MenuProvider):
    ...
    def get_file_items(self, *args):
        ...
        menu_item = Nautilus.MenuItem(
            name="GitRepo::Open", label=label, tip=_("Manage this Git repository with GitRepo")
        )
    ...
    def get_background_items(self, *args):
        ...
        label = _("Open in GitRepo")
        menu_item = Nautilus.MenuItem(
            name="GitRepo::OpenBackground", label=label, tip=_("Manage this Git repository with GitRepo")
        )
        # Pass the git root as the path to open
        menu_item.connect("activate", self._launch_application_path, git_root)
        return [menu_item]
    ...
    def _launch_application_path(self, menu_item, file_path: str):
        ...
        if not file_path or not Path(file_path).exists():
            self._show_notification(_("Error"), _("Could not get the path for the selected folder."))
            return
        ...
        try:
            ...
            self._show_notification(_("Application Not Found"), _("Could not find GitRepo. Is it installed?"))
        except Exception as e:
            print(f"GitRepo Extension: Error launching: {e}")
            self._show_notification(_("Launch Error"), str(e))
    ...
    def _show_notification(self, title: str, message: str):
        ...
        try:
            subprocess.run(
                ["notify-send", "--icon=dialog-error", f"--app-name={APP_NAME}", title, message], check=False
            )
        except FileNotFoundError:
