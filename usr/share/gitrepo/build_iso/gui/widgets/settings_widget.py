...
from gitrepo.build_iso.core.config import VALID_BRANCHES
...
class SettingsWidget(Gtk.Box):
    ...
    def _create_ui(self):
        ...
        general_group.set_title(_("Storage"))
        general_group.set_description(_("Default destination for generated images and build artifacts."))
        page_content.append(general_group)
        ...
        browse_btn.update_property([Gtk.AccessibleProperty.LABEL], [_("Select Output Directory")])
        ...
        container_group.set_description(_("Runtime and image used for isolated ISO builds."))
        ...
        defaults_group.set_title(_("Package Channels"))
        defaults_group.set_description(_("Branches selected automatically when a new build is configured."))
        defaults_group.set_margin_top(24)
        page_content.append(defaults_group)
        ...
        notif_group.set_description(_("Choose how GitRepo reports the end of a build."))
        notif_group.set_margin_top(24)
        page_content.append(notif_group)
    ...
    def _save_output_dir(self):
        ...
        if text:
            if self.settings.set("general", "output_dir", text):
    ...
    def _on_save_clicked(self, button):
        ...
        root = self.get_root()
        if self.settings.set_many(
            {
                ("general", "output_dir"): self.output_row.get_text(),
                ("general", "branches"): branches,
                ("container", "image"): self.image_row.get_text(),
                ("container", "engine"): "docker" if self.engine_row.get_selected() == 0 else "podman",
                ("notifications", "desktop"): self.notify_complete_row.get_active(),
                ("notifications", "sound"): self.notify_sound_row.get_active(),
            }
        ):
            self.emit("settings-saved")
            if root and hasattr(root, "show_toast"):
